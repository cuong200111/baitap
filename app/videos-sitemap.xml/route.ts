    ...
    <lastmod>${new Date(item.lastmod).toISOString()}</lastmod>`;
      ...
      item.videos.forEach((video: any) => {
        ...
        const thumbnailUrl = video.thumbnail?.startsWith("http")
          ? video.thumbnail
          : `${baseUrl}${video.thumbnail || item.thumbnail}`;
      ...
      videos: [
        {
          ...
          description:
            "Video giới thiệu về HACOM, hệ thống bán máy tính, laptop uy tín tại Việt Nam",
      ...
      videos: [
        {
          ...
          description:
            "Video hướng dẫn chi tiết cách chọn laptop gaming phù hợp v���i nhu cầu và ngân sách",
