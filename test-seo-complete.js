...
async function makeRequest(endpoint, options = {}) {
  ...
  try {
    ...
    console.log(
      `${status} ${endpoint}: ${response.status} - ${data.success ? "Success" : data.message || "Failed"}`,
    );
...
async function testSeoEndpoints() {
  ...
  await makeRequest("/api/admin/seo-ai-recommendations", {
    method: "POST",
    body: JSON.stringify({ analysisType: "all", timeframe: "30d" }),
  });
  ...
  await makeRequest("/api/admin/seo-content-analysis", {
    ...
    body: JSON.stringify({
      content:
        "<h1>Test Content</h1><p>This is a test content for SEO analysis with gaming laptop keywords.</p>",
  ...
  await makeRequest("/api/admin/seo-auto-fix", {
    method: "POST",
    body: JSON.stringify({
      issues: ["missing_alt_text", "missing_meta_description"],
    }),
  });
