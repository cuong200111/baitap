...
async function testStockValidation() {
  ...
  try {
    ...
    if (!data3.success) {
      console.log(
        "✅ Third add failed as expected with message:",
        data3.message,
      );
      if (data3.current_in_cart !== undefined) {
        console.log(
          "✅ New response format includes current_in_cart:",
          data3.current_in_cart,
        );
    ...
    if (data4.success) {
      console.log("✅ Cart contents:", {
        ...
        total_quantity: data4.data.items.reduce(
...
function testFormatPrice() {
  ...
  testCases.forEach((testCase) => {
    try {
      ...
      console.log(
        `❌ formatPrice failed for ${JSON.stringify(testCase)}:`,
        error.message,
      );
    }
  });
