...
export async function generateAdminMetadata(options: {
  ...
  let finalDescription = String(
    description ||
      adminSeoData.general.site_description ||
  ...
  const finalImage = String(
    image || adminSeoData.social.default_og_image || "/og-image.jpg",
  );
  ...
  return {
    ...
    keywords: String(
      keywords ||
        adminSeoData.general.site_keywords ||
        "máy tính, laptop, gaming",
    ),
    authors: [
      { name: String(adminSeoData.schema.organization_name || "HACOM") },
    ],
    ...
    verification: {
      google: String(
        adminSeoData.analytics.google_search_console_verification || "",
      ),
    },
    other: {
