...
async function testOrdersAPI() {
  try {
    ...
    const createOrderResponse = await fetch(`${BASE_URL}/api/debug/create-test-order-complete`, {
      method: 'POST',
    ...
    const createOrder2Response = await fetch(`${BASE_URL}/api/debug/create-test-order-complete`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' }
    });
    ...
    if (!loginResponse.ok) {
      ...
      // Try to create admin
      const createAdminResponse = await fetch(`${BASE_URL}/api/auth/create-admin`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
