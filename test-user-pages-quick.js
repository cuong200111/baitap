...
async function testAllPages() {
  ...
  console.log(`⭐ Critical pages: ${userPages.filter(p => p.critical).length}\n`);
  ...
  for (let i = 0; i < userPages.length; i++) {
    ...
    process.stdout.write(`Testing ${i + 1}/${userPages.length}: ${page.name} (${page.url})... `);
    ...
    if (result.success) {
      ...
      errorCount++;
      console.log(`❌ FAILED (${result.status || 'ERROR'}) - ${result.error || 'Unknown error'}`);
    }
  ...
  console.log(`📊 Success Rate: ${((successCount / userPages.length) * 100).toFixed(1)}%`);

  if (errorCount > 0) {
    console.log('\n❌ FAILED PAGES:');
    results.filter(r => !r.success).forEach(page => {
      console.log(`  • ${page.name} (${page.url}): ${page.error || `HTTP ${page.status}`}${page.critical ? ' ⭐ CRITICAL' : ''}`);
    });
