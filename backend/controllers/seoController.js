...
export const seoController = {
  ...
  try {
    ...
    const defaultSettings = {
      ...
      site_description: "Leading computer and gaming equipment store in Vietnam",
  ...
  try {
    ...
    if (existing.length > 0) {
      await executeQuery(
        ...
        [site_title, site_description, keywords, robots_txt, meta_author, og_image]
      );
    } else {
      await executeQuery(
...
async saveSeoSettings(req, res) {
  ...
  try {
    const testResults = {
      technical_seo: {
        robots_txt: { status: "pass", message: "Robots.txt is properly configured" },
        sitemap: { status: "pass", message: "Sitemap is accessible and valid" },
        meta_tags: { status: "warning", message: "3 pages missing meta description" },
        structured_data: { status: "pass", message: "Schema markup is properly implemented" },
      },
      on_page_seo: {
        title_tags: { status: "pass", message: "All pages have unique title tags" },
        headings: { status: "pass", message: "Proper heading structure" },
        images: { status: "warning", message: "5 images missing alt text" },
        internal_links: { status: "pass", message: "Good internal linking structure" },
      },
      performance: {
        page_speed: { status: "pass", message: "Page speed is acceptable" },
        mobile_friendly: { status: "pass", message: "Site is mobile responsive" },
        core_web_vitals: { status: "warning", message: "LCP could be improved" },
      },
      overall_score: 87,
      status: "good",
    };
