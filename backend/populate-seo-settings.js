...
async function populateDefaultSeoSettings() {
  try {
    ...
    const defaultSettings = [
      ...
      ['site_keywords', 'máy tính, laptop, gaming, linh kiện máy tính, PC, HACOM', 'general'],
      ...
      ['product_meta_title_pattern', '{product_name} - {category} | HACOM', 'general'],
      ['category_meta_title_pattern', '{category_name} - {description} | HACOM', 'general'],
      ...
      ['organization_address', 'Số 131 Lê Thanh Nghị, Hai Bà Trưng, Hà Nội', 'schema'],
    ...
    console.log(`✅ Successfully inserted ${defaultSettings.length} SEO settings`);

    // Also populate some default site settings
    const siteSettings = [
      ['general_site_name', 'HACOM E-commerce', 'string'],
      ['general_site_description', 'Chuyên cung cấp laptop, PC gaming và linh kiện máy tính', 'string'],
    ...
    if (siteSettingsCount[0].count === 0) {
      ...
      console.log(`✅ Successfully inserted ${siteSettings.length} site settings`);
    } else {
...
// Run the script
populateDefaultSeoSettings().then(() => {
