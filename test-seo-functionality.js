  ...
  try {
    ...
    if (settingsResponse.ok) {
      ...
      if (settingsData.data?.general?.site_name) {
        ...
        console.log('📄 Description length:', settingsData.data.general.site_description?.length || 0);
      }
    } else {
    ...
    if (statusResponse.ok) {
      ...
      console.log('🔍 SEO settings count:', statusData.data?.checks?.seoSettings || 0);
    } else {
    ...
    if (websiteResponse.ok) {
      ...
      if (descMatch) {
        console.log('✅ Meta description found:', descMatch[1].substring(0, 50) + '...');
      } else {
    ...
    const analysisResponse = await fetch(`${Domain}/api/seo/content-analysis`, {
      ...
      body: JSON.stringify({
        content: 'Laptop gaming ASUS ROG với hiệu năng mạnh mẽ. Máy tính chuyên game với card đồ họa RTX 4060. Giá tốt nhất thị trường.',
    ...
    if (analysisResponse.ok) {
      ...
      console.log('🔤 Keyword density:', analysisData.data?.keywordDensity || {});
    } else {
